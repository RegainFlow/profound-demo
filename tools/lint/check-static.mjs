...
for (const root of scannedRoots) {
  for (const file of await listFiles(path.join(repoRoot, root))) {
    ...
    if (source.includes('console.log(') && !relative.startsWith('backend/server.mjs') && !relative.startsWith('tools/')) {
...
async function listFiles(directory) {
  ...
  return files.flat().filter((file) => /\.(mjs|js|html|css|md)$/.test(file));
}
